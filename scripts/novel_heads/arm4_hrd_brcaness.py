...
_HRD_COMPOSITE_ROW = "HRD"  
_HRD_COMPONENT_ROWS = ("ai1", "lst1", "hrd-loh")  
...
_SUBTYPE_BASAL = ("BRCA_Basal",)  
_SUBTYPE_LUMINAL = ("BRCA_LumA", "BRCA_LumB")  
_MIN_STRATUM_N = 80
...
def _build_matrices(cfg: ArmConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.Series, int]:
    ...
    if len(shared) < 2 * len(cfg.seeds) * 5:  
        raise ValueError(
...
def _run_deconfound_cli(cfg: ArmConfig, args) -> int:
    ...
    primary = outcome.strata[0] if outcome.strata else None
    print(f"arm4 de-confound control (subtype-stratified HRD) — appended: {closeout_path}")  
    for s in outcome.strata:
        if s.result is not None:
            auroc = _fmt_auroc(s.result.auroc)
            ci = (
...
class ContinuousRegressionResult:
    ...
    boot_ci95_mean: tuple[float, float]  
...
def run_arm4_continuous_regression(cfg: ArmConfig) -> ContinuousRegressionResult:
    ...
    y_cont = score_ser.to_numpy(dtype=float)  
...
def _run_continuous_regression_cli(cfg: ArmConfig, args) -> int:
    print("arm4 --continuous-regression: running elastic-net regression on raw HRD score...")  
    print(  
    ...
    print(f"arm4 continuous-regression — report: {md_path}")  
    print(f"arm4 continuous-regression — json:   {json_path}")  
    print(  
...
def run_arm4_advance(cfg: ArmConfig) -> tuple[dict, dict, list[dict]]:
    ...
    print("arm4 --advance: re-running floor gate to obtain OOF predictions for calibration...")  
    print("  (elastic-net SAGA, ~25-35 min total — LOCK-2 floor, do not reduce seeds)")  
    result = run_floor_gate(x, y, groups, model=_MODEL, seeds=cfg.seeds)

    print("arm4 --advance: running OOF calibration (isotonic)...")  
    cal_iso = oof_calibrate(result, method="isotonic")
    print("arm4 --advance: running OOF calibration (Platt)...")  
    cal_platt = oof_calibrate(result, method="platt")

    print("arm4 --advance: fitting full-data model (seed=0) for SHAP LinearExplainer...")  
    ...
    print("arm4 --advance: computing SHAP values (LinearExplainer on full feature matrix)...")  
...
def _run_advance_cli(cfg: ArmConfig, args) -> int:
    ...
    try:
        ...
        print(f"CALIBRATION TRIPWIRE FIRED: {exc}", file=sys.stderr)  
        print(  
    ...
    print(f"arm4 --advance — advance report: {advance_md_path}")  
    print(f"arm4 --advance — advance JSON:   {advance_json_path}")  
    print(f"arm4 --advance — SHAP CSV:       {csv_path}")  
    if png_path:
        print(f"arm4 --advance — SHAP PNG:       {png_path}")  
    print(f"arm4 --advance — appended to:    {closeout_path}")  
    print(  
    ...
    print(f"  SHAP top-5: {[d['feature'] for d in shap_list[:5]]}")  
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.continuous_regression:
        try:
            ...
            print(f"BLOCKED (continuous-regression): {exc}", file=sys.stderr)  
            return 2
    ...
    shuf = "NaN" if not np.isfinite(outcome.shuffle_auroc) else f"{outcome.shuffle_auroc:.3f}"
    print(f"arm4 genuine-HRD floor gate — report: {report_path}")  
    print(  
