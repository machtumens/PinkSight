...
from pinksight.data.slice_dataset_diag import FULLN_DIR, DiagSliceGradeDataset
...
def _assert_slice_disjoint(ds_a: DiagSliceGradeDataset, ds_b: DiagSliceGradeDataset, where: str) -> None:
...
def cross_val_slices_diag(
    ...
    def _loader(pairs, split, augment, shuffle, seed, fold):
        ds = DiagSliceGradeDataset(pairs, source=source, proc_dir=proc_dir, split=split, augment=augment)
        gen = None
        if shuffle:
            import torch
    ...
    for seed in seeds:
        ...
        for f, (tr, te) in enumerate(cv.split(np.zeros(n), y, pids)):
            ...
            _best, oof_probs, oof_pids = train_model(
                ...
                pos_weight=neg / max(pos, 1.0), log_tag=f"{source}_s{seed}f{f}",
    ...
    return {
        ...
        "slice_level_patient_disjoint_asserted": True,
    }
